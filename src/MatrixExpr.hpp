#ifndef METRIX_EXPRESSION
#define METRIX_EXPRESSION
...
namespace minimath {
	...
	template <typename Scaler, typename A>
	auto operator*(const Scaler& s, const MatrixExpr<A>& a) {
		return ScaleMatExpr<Scaler, A>(s, a.derived());
	}

	template <typename A>
	class TransposeMatExpr : public MatrixExpr<TransposeMatExpr<A>> {
		A a_;
	public:
	...
	template <typename A>
	auto transpose(const MatrixExpr<A>& a) {
		return TransposeMatExpr(a.derived());
	}

	template <typename A, typename B>
	class MatMulExpr : public MatrixExpr<MatMulExpr<A,B>> {
		A a_;
		B b_;
	public:
